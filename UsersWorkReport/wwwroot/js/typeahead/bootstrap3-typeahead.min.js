!function (t, e) {
    ...
    "use strict";
    var e = function (s, i) {
        this.$element = t(s), this.options = t.extend({}, e.defaults, i), this.matcher = this.options.matcher || this.matcher,
            ...
            this.itemLink = this.options.itemLink || this.itemLink, this.itemTitle = this.options.itemTitle || this.itemTitle,
            this.followLinkOnSelect = this.options.followLinkOnSelect || this.followLinkOnSelect,
            this.source = this.options.source, this.delay = this.options.delay, this.theme = this.options.theme && this.options.themes && this.options.themes[this.options.theme] || e.defaults.themes[e.defaults.theme],
            this.$menu = t(this.options.menu || this.theme.menu), this.$appendTo = this.options.appendTo ? t(this.options.appendTo) : null,
            ...
            this.afterSelect = this.options.afterSelect, this.afterEmptySelect = this.options.afterEmptySelect,
            this.addItem = !1, this.value = this.$element.val() || this.$element.text(), this.keyPressed = !1,
            this.focused = this.$element.is(":focus"), this.changeInputOnSelect = this.options.changeInputOnSelect || this.changeInputOnSelect,
            this.changeInputOnMove = this.options.changeInputOnMove || this.changeInputOnMove,
            this.openLinkInNewTab = this.options.openLinkInNewTab || this.openLinkInNewTab,
            this.selectOnBlur = this.options.selectOnBlur || this.selectOnBlur, this.showCategoryHeader = this.options.showCategoryHeader || this.showCategoryHeader;
    };
    e.prototype = {
        constructor: e,
        ...
        select: function () {
            ...
            if (this.$element.data("active", t), this.autoSelect || t) {
                var e = this.updater(t);
                e || (e = ""), this.changeInputOnSelect && this.$element.val(this.displayText(e) || e).text(this.displayText(e) || e).change(),
                    this.followLinkOnSelect && this.itemLink(t) ? (this.openLinkInNewTab ? window.open(this.itemLink(t), "_blank") : document.location = this.itemLink(t),
                        this.afterSelect(e)) : this.followLinkOnSelect && !this.itemLink(t) ? this.afterEmptySelect(e) : this.afterSelect(e);
        ...
        process: function (e) {
            ...
            return e = t.grep(e, function (t) {
                ...
                "all" != this.options.items && (e = e.slice(0, this.options.items)), this.options.addItem && e.push(this.options.addItem),
        ...
        render: function (e) {
            ...
            return t.each(e, function (t, s) {
                ...
                return (e = t(s.options.item || s.theme.item).data("value", n)).find(s.options.itemContentSelector || s.theme.itemContentSelector).addBack(s.options.itemContentSelector || s.theme.itemContentSelector).html(s.highlighter(h, n)),
                    s.options.followLinkOnSelect && e.find("a").attr("href", i.itemLink(n)), e.find("a").attr("title", i.itemTitle(n)),
        ...
        next: function (e) {
            this.$menu.find(".active").removeClass('dropdown-item-client1');
            ...
            s.addClass("active");
            this.$menu.find(".active").addClass('dropdown-item-client1');
        ...
        listen: function () {
            this.$element.on("focus.bootstrap3Typeahead", t.proxy(this.focus, this)).on("blur.bootstrap3Typeahead", t.proxy(this.blur, this)).on("keypress.bootstrap3Typeahead", t.proxy(this.keypress, this)).on("propertychange.bootstrap3Typeahead input.bootstrap3Typeahead", t.proxy(this.input, this)).on("keyup.bootstrap3Typeahead", t.proxy(this.keyup, this)),
        ...
        destroy: function () {
            this.$element.data("typeahead", null), this.$element.data("active", null), this.$element.unbind("focus.bootstrap3Typeahead").unbind("blur.bootstrap3Typeahead").unbind("keypress.bootstrap3Typeahead").unbind("propertychange.bootstrap3Typeahead input.bootstrap3Typeahead").unbind("keyup.bootstrap3Typeahead"),
                this.eventSupported("keydown") && this.$element.unbind("keydown.bootstrap3-typeahead"),
                this.$menu.remove(), this.destroyed = !0;
        },
        ...
        keydown: function (e) {
            17 !== e.keyCode && (this.keyPressed = !0, this.suppressKeyPressRepeat = ~t.inArray(e.keyCode, [40, 38, 9, 13, 27]),
        ...
        blur: function (t) {
            this.mousedover || this.mouseddown || !this.shown ? this.mouseddown && (this.skipShowHintOnFocus = !0,
                this.$element.focus(), this.mouseddown = !1) : (this.selectOnBlur && this.select(),
                    this.hide(), this.focused = !1, this.keyPressed = !1);
        },
        click: function (t) {
    ...
    t.fn.typeahead = function (s) {
        ...
        themes: {
            bootstrap3: {
                menu: '<ul class="typeahead dropdown-menu dropdown-menu-client" role="listbox"></ul>',
                item: '<li><a class="dropdown-item dropdown-item-client" href="#" role="option"></a></li>',
                itemContentSelector: "a",
                headerHtml: '<li class="dropdown-header"></li>',
                headerDivider: '<li class="divider" role="separator"></li>'
            },
            bootstrap4: {
                menu: '<div class="typeahead dropdown-menu dropdown-menu-client" role="listbox"></div>',
                item: '<button class="dropdown-item dropdown-item-client" role="option"></button>',
...
function CheckSearchValue(inputId, selectedId) {
    if ($('#' + inputId).val() == '') {
        $('#' + selectedId).val('');
    }
}
function autoCompleteType(inputId, url, minLength, selectedId, param1) {
    $('#' + inputId).typeahead({
        hint: true,
        highlight: true,
        minLength: minLength,
        source: function (request, response) {
            ...
            $.ajax({
                url: url,
                data: { prefix: request, param1: param },
                type: "POST",
                success: function (data) {
                    ...
                    $.each(data, function (i, item) {
                        ...
                        map[itemtext] = { itemtext: itemtext, itemvalue: itemvalue, inputId: inputId };
                        items.push(itemtext);
                    });
                    response(items);
                    $(".dropdown-menu").css("height", "auto");
                    $(".dropdown-menu").find(".active").addClass('dropdown-item-client1');
                },
        ...
        afterSelect: function (item) {
            var id = map[item].itemvalue;
            $('#' + selectedId).val(id);
            if (window.GetSelectedItemCode) {
                GetSelectedItemCode(id, map[item].inputId);
            }
        }
    });
}